...
export function Dashboard() {
    ...
    const fetchCryptoData = async () => {
      try {
        ...
        const formattedData = response.data.map(crypto => ({
          ...
          name: crypto.name || crypto.symbol || 'Unknown',
          currentPrice: crypto.currentPrice || 0,
          priceChange: crypto.priceChangePercent || 0,
          volume: crypto.volume || 'N/A'
        }));

        setCryptoData(formattedData);
        
        if (formattedData.length > 0) {
          const firstCrypto = formattedData[0];
          setSelectedCoin({
  ...
  const handleShowMore = () => {
    if (visibleCount === 4) {
      setVisibleCount(14); // Show 4 + 10 more
    } else {
                  ...
                  >
                    ...
                    <div className="text-right">
                      ...
                      <p className={`text-sm flex items-center justify-end ${
                        ...
                        {Number(coin.priceChange).toFixed(2)}%
                      </p>
                    </div>
                  </motion.div>
                ))
